...
def get_spreadsheet():
    ...
    # Prefer env spreadsheet name; fall back to config constant
...
def get_worksheet(tab_name: str):
    ...
    try:
        ...
        # Create missing worksheet with header if known
...
def load_categories_df() -> pd.DataFrame:
    ...
    if df.empty:
        # Seed from config definitions if available
        items = []
...
def load_toggles() -> Dict[str, Any]:
    # store toggles in baseline sheet with a prefix, to avoid another tab
    base = load_baseline()
